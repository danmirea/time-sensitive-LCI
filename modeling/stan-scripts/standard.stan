/*
Title: Model 21: vanilla CRP, Gaussian likelihood, one size prior
Date: 30 Feb 2023
Authors: Dan-Mircea Mirea
Description: individual Stan model for the QIP (galaxy) task; each individual parameter is drawn from a population-level gamma distribution 

Parameters (2):
-CRP concentration: alpha
-size prior: sigma (variance of Gaussian generalization functions)
*/
...
model {
    ...
    // concentration
    alpha_ind ~ gamma(0.730, 1.75);

    // size prior means for both dimensions
    sigma_ind_pre ~ gamma(2.50, 38.25);
